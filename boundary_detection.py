...
def estimate_corners_with_hough(edges, gray_image):
    ...
    :return: 估算的四个角点 (numpy array) 或 None
    """
    ...
    if horizontal_lines:
        ...
        top_line = horizontal_lines[0] if len(horizontal_lines) > 0 else None
        bottom_line = horizontal_lines[-1] if len(horizontal_lines) > 0 else None
    
    if vertical_lines:
        vertical_lines = sorted(vertical_lines, key=lambda l: (l[0] + l[2]) / 2)
        left_line = vertical_lines[0] if len(vertical_lines) > 0 else None
        right_line = vertical_lines[-1] if len(vertical_lines) > 0 else None
